...
public class BlogService {
     ...
     * @throws IOException
     */
    public synchronized String saveFile(MultipartFile file) throws IOException {
...
//     * @param blogTitle
    ...
    public void saveBlog(Blog blog, Integer[] tagIds) throws JsonProcessingException {
...
//        blog.setBody(blogBody);
     ...
     * @return
     */
    public Long getHomeBlogCountByTag(Integer tagId) {
        return blogDao.getHomeBlogCountByTag(tagId);
    }
     ...
     * @return
     */
    public List<Blog> findHomeBlogByTag(Integer page, Integer showCount, Integer tagId) {        // mysql 分页中的开始位置
        int start = (page - 1) * showCount;

        // 返回的blog列表
        List<Blog> blogs = new LinkedList<>();
        blogs = blogDao.findHomeBlogByTag(start, showCount, tagId);
        for (Blog blog : blogs) {
...
//        if (tagID == null)
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void updateBlog(Integer blogId, Blog blog, Integer[] tagIds) throws JsonProcessingException {
        User user = userDao.findUserByName(jwtTokenUtil.getUsernameFromRequest(request));
        Blog blog1 = blogDao.findBlogById(blogId);
        if (!user.getId().equals(blog1.getUser().getId())) {
            throw new RuntimeException("无权限修改");
        }
        Date meetingData = blog.getMeetingData();
        blog.setMeetingData(meetingData);
//        blog.setId(blogId);
...
//        blog.setBody(blogBody);
        ...
        if (redisTemplate.hasKey(RedisConfig.REDIS_BLOG_PREFIX + blogId)) {
            //重新取数放到redis
            Blog blog2 = blogDao.findBlogById(blogId);
            blog2.setTags(tagDao.findTagByBlogId(blogId));
//            blog.setUser(user);
            redisTemplate.opsForValue().set(RedisConfig.REDIS_BLOG_PREFIX + blogId, objectMapper.writeValueAsString(blog2));
        }
    }
