  ...
  <el-card id="blogOverView">
    ...
    >
      ...
      <span
        ...
        v-if="
          show &&
          (getStoreName() == name || getStoreRoles().indexOf('ADMIN') > -1)
        "
        title="删除"
        @click="deleteBlog(id)"
      >
    ...
    <div @mouseenter="pEnter()" @mouseleave="pLeave()">
      <p style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap">
        <span class="el-icon-time hidden-xs-only">
          <span> 会议时间： </span>
          <span v-if="meetingData!='NaN-aN-aN'"> &nbsp;{{ meetingData }} </span>
        </span>

        <span
          class="el-icon-location-information hidden-xs-only"
          style="margin-left: 25px"
          >会议地点：&nbsp;{{ meetingArea }}</span
        >

        <span class="el-icon-user hidden-xs-only" style="margin-left: 25px"
          >领导人：&nbsp;{{ leader }}</span
        >

        <span class="el-icon-view hidden-xs-only" style="margin-left: 25px"
          >&nbsp;{{ blogViews }}</span
        >

        <span
          class="el-icon-edit hidden-xs-only"
        ...
        <span style="margin-left: 25px">
          ...
          <span v-for="tag in tags" :key="tag.id">
            <el-tag size="small" type="success" style="margin-left: 15px">{{
              tag
            }}</el-tag>
          </span>
        </span>
