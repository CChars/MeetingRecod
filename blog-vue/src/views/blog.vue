<template>
  <div>
    <el-card id="blog">
      ...
      <div style="text-align: center">
        <p>
          <span class="el-icon-s-custom" style="margin-left: 60px"
            >领导人：</span
          >
          <span class="hidden-xs-only" v-if="ruleForm.leader != null"
            >&nbsp;{{ ruleForm.leader }}</span
          >

          <span class="el-icon-s-check" style="margin-left: 60px"
            >参会人：</span
          >
          <span class="hidden-xs-only">&nbsp;{{ ruleForm.attendance }}</span>

          <span class="el-icon-watch" style="margin-left: 60px"
            >会议时间：</span
          >
          <span class="hidden-xs-only" v-if="ruleForm.meetingData != null"
            >&nbsp;{{ ruleForm.meetingData | formatDate }}</span
          >

          <span class="el-icon-map-location" style="margin-left: 60px"
            >会议地点：</span
          >
          <span class="hidden-xs-only">&nbsp;{{ ruleForm.meetingArea }}</span>
        </p>
      </div>

      <div style="text-align: center">
        <p>
          <span class="el-icon-notebook-1" style="margin-left: 30px"
            >论文文档：</span
          >
          <span class="hidden-xs-only" v-if="ruleForm.paperURL != null"
            >&nbsp;{{ paperName }}</span
          >
          <a
            :href="ruleForm.paperURL"
            target="_blank"
            v-if="ruleForm.paperURL != null"
            >点击下载文件</a
          >

          <span class="el-icon-data-board" style="margin-left: 30px"
            >PPT文档：</span
          >
          <span class="hidden-xs-only" v-if="ruleForm.pptURL != null"
            >&nbsp;{{ pptName }}</span
          >
          <a
            :href="ruleForm.pptURL"
            target="_blank"
            v-if="ruleForm.pptURL != null"
            >点击下载文件</a
          >

          <span class="el-icon-link" style="margin-left: 30px"
            >会议视频链接：</span
          >
          <!-- <span class="hidden-xs-only" @click="openLink"
            >&nbsp;{{ ruleForm.meetingVideoLink }}</span
          > -->
          <a
            :href="ruleForm.meetingVideoLink"
            target="_blank"
            v-if="ruleForm.meetingVideoLink != null"
            >视频链接</a
          >
        </p>
        <p>
  ...
  methods: {
    ...
    loadBlog() {
      ...
      blog.getBlogById(this.blogId, isClick).then((res) => {
        ...
        console.log("查看shij：", res.data.meetingData);
        this.ruleForm.meetingArea = res.data.meetingArea;
        this.ruleForm.leader = res.data.leader;
        this.ruleForm.meetingData = res.data.meetingData;
        this.ruleForm.attendance = res.data.attendance;
        this.ruleForm.meetingVideoLink = res.data.meetingVideoLink;
        //文件链接
        this.ruleForm.paperURL = res.data.paperURL;
        this.ruleForm.pptURL = res.data.pptURL;
        var AllPaperURL = res.data.paperURL;
        var AllPptURL = res.data.pptURL;
        console.log(
          "论文名称：",
          AllPaperURL.split("/")[AllPaperURL.split("/").length - 1]
        );
        console.log(
          "ppt名称：",
          AllPptURL.split("/")[AllPptURL.split("/").length - 1]
        );
        this.paperName = AllPaperURL.split("/")[
          AllPaperURL.split("/").length - 1
        ];
        this.pptName = AllPptURL.split("/")[AllPptURL.split("/").length - 1];
      ...
      discuss
        .getDiscussByBlogId(this.blogId, this.currentPage, this.pageSize)
        .then((responese) => {
    ...
    openLink() {
      window.open(this.ruleForm.meetingVideoLink);
    },
    ...
    likeThis() {
      if (this.isLogin()) {
        userLike
          .saveUserLike(this.blogId, this.like == 0 ? 1 : 0)
          .then((res) => {
